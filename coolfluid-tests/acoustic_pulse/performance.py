...
def do_plot(order,file_names,rk_names,stages):
    ref_scheme_cfl, ref_scheme_eff_dt, ref_scheme_LInfinity, ref_scheme_cpu = read_file(order,file_names[0],stages[0])
    small_cfl, small_eff_dt, small_LInfinity, small_cpu = read_file(order,file_names[1],stages[1])
    large_cfl, large_eff_dt, large_LInfinity, large_cpu = read_file(order,file_names[2],stages[2])
    ...
    # Save figure
    plt.savefig("../../figures/acoustic_pulse/error-cfl-selected-optimal-erk"+order+".eps")
